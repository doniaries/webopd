<div class="w-full max-w-[300px] mx-auto rounded-lg shadow-md overflow-hidden h-[390px] relative bg-gray-100">
        ...
        <div class="swiper-wrapper h-full">
            @forelse ($banners as $banner)
                <div class="swiper-slide h-full">
                    <div class="relative w-full h-full overflow-hidden group">
                        @php
                            // Debugging info
                            $storagePath = 'banners/' . basename($banner->gambar);
                            $imageExists = $banner->gambar && \Illuminate\Support\Facades\Storage::exists($storagePath);
                            $imageUrl = $imageExists ? \Illuminate\Support\Facades\Storage::url($storagePath) : null;
                            
                            \Illuminate\Support\Facades\Log::info('Banner Data:', [
                        ...
                        <a href="{{ $banner->url ?? '#' }}" class="block w-full h-full">
                            <img 
                                src="{{ $imagePath }}" 
                                alt="{{ $banner->judul }}"
                                class="w-full h-full object-cover transition-all duration-700 ease-in-out transform hover:scale-105"
            ...
            @empty
                <div class="swiper-slide">
                    <div
                        class="w-full h-full flex flex-col items-center justify-center bg-gray-100 text-gray-500 p-4 text-center">

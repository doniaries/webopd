    ...
    <div class="container mx-auto px-4 py-8">
                ...
                <div class="relative">
                    <span class="absolute inset-y-0 left-0 flex items-center pl-3">
                        <svg class="h-5 w-5 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 4h13M3 8h9m-9 4h6m4-4h6m-6 4h6m-6 4h6m4-8h4m-4 4h4m-4 4h4" />
                        </svg>
                    </span>
                ...
                <div class="mt-4 flex flex-wrap items-center gap-2">
                    ...
                    @if($search)
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                            Pencarian: {{ $search }}
                            <button type="button" wire:click="$set('search', '')" class="ml-1.5 inline-flex items-center justify-center h-4 w-4 rounded-full bg-blue-200 hover:bg-blue-300 focus:outline-none focus:bg-blue-300">
                                <span class="sr-only">Hapus filter pencarian</span>
                                <svg class="h-2 w-2 text-blue-800" stroke="currentColor" fill="none" viewBox="0 0 8 8">
                    ...
                    @if($category)
                        @php
                            $categoryName = $categories->firstWhere('slug', $category)->name ?? $category;
                        @endphp
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                            Kategori: {{ $categoryName }}
                            <button type="button" wire:click="$set('category', '')" class="ml-1.5 inline-flex items-center justify-center h-4 w-4 rounded-full bg-green-200 hover:bg-green-300 focus:outline-none focus:bg-green-300">
                                <span class="sr-only">Hapus filter kategori</span>
                                <svg class="h-2 w-2 text-green-800" stroke="currentColor" fill="none" viewBox="0 0 8 8">
                                    <path stroke-linecap="round" stroke-width="1.5" d="M1 1l6 6m0-6L1 7" />
                                </svg>
                            </button>
                        </span>
                    ...
                    @if($sort !== 'latest')
                        ...
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-purple-100 text-purple-800">
                            Urutkan: {{ $sortLabels[$sort] ?? $sort }}
                            <button type="button" wire:click="$set('sort', 'latest')" class="ml-1.5 inline-flex items-center justify-center h-4 w-4 rounded-full bg-purple-200 hover:bg-purple-300 focus:outline-none focus:bg-purple-300">
                                <span class="sr-only">Hapus filter pengurutan</span>
                                <svg class="h-2 w-2 text-purple-800" stroke="currentColor" fill="none" viewBox="0 0 8 8">
                                    <path stroke-linecap="round" stroke-width="1.5" d="M1 1l6 6m0-6L1 7" />
                                </svg>
                            </button>
                        </span>
                    @endif
                    <button type="button" wire:click="resetFilters" class="ml-2 text-sm text-blue-600 hover:text-blue-800 focus:outline-none">
                        Reset semua
                    </button>
                </div>
            @endif
        ...
        <div class="flex justify-between items-center mb-6">
            ...
            <div class="flex space-x-2">
                <button wire:click="$set('layout', 'grid')" class="p-2 rounded-md {{ $layout === 'grid' ? 'bg-blue-100 text-blue-600' : 'bg-gray-100 text-gray-600' }}" title="Tampilan Grid">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z" />
                    </svg>
                </button>
                <button wire:click="$set('layout', 'list')" class="p-2 rounded-md {{ $layout === 'list' ? 'bg-blue-100 text-blue-600' : 'bg-gray-100 text-gray-600' }}" title="Tampilan Daftar">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        @if($post->category)
                            @php
                                ...
                                $colors = [
                                    ...
                                    'Pengumuman' => 'bg-yellow-100 text-yellow-800 hover:bg-yellow-200',
                                    'default' => 'bg-gray-100 text-gray-800 hover:bg-gray-200'
                                ];
                                $categoryClass = $colors[$post->category->name] ?? $colors['default'];
                            @endphp
                            <a href="{{ route('posts.index', ['category' => $post->category_id]) }}" 
                               class="inline-block text-sm font-medium px-3 py-1 rounded-full mb-3 transition-colors {{ $categoryClass }}">
                                {{ $post->category->name }}
                            </a>
                        @endif
        ...
        <article class="max-w-4xl mx-auto">
            ...
            @if($post->foto_utama)
                <div class="mb-8 rounded-lg overflow-hidden">
                    <img src="{{ asset('storage/foto-utama/' . $post->foto_utama) }}" 
                         alt="{{ $post->title }}" 
